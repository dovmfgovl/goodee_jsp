...
public class NoticeController implements Action {
	...
	public ActionForward execute(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		////////////////////<< 사용자로 부터 입력받은 값 처리 >>///////////////////
		if("noticeList".equals(upmu[1])) {//select : 1-3버전에서는 이 장면을 메소드 단위로 변경하고 싶다. (req, res) 넘겨 받을 수 있어야 한다 <- 이 문제를 해결해야함
		...
		//파일 업로드 시와 마찬가지로 파일 정보를 얻어올때도 출력페이지를 내보낼 필요가 없음
		if(path != null) { //응답페이지가 존재하는 경우만 처리할 것
			af.setPath(path.toString());	//이대로 두면 NullPointerException 대상임(path에 null 넣었으므로)					
		} else {
			af.setPath(null);//이대로 두면 NullPointerException 대상임(path에 null 넣었으므로)								
		}
