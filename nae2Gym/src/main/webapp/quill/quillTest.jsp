		    ...
		    fileInput.addEventListener("change", function () {  // change 이벤트로 input 값이 바뀌면 실행
		        ...
		        $.ajax({//<form method=post enctype=multipart/form-data/> 이럴경우 req.getParameter사용이 불가함 -> cos.jar
		            ...
		            success: function (response) {
						...
						const url = "http://localhost:8000/notice/imageGet.gd?imageName="+response;// + style=~ 붙음
						const range = quill.getSelection().index;
						quill.setSelection(range, 1);
						quill.clipboard.dangerouslyPasteHTML(range, '<img src='+url+' style="width:100%; height: auto;" alt="image"/>'); //사진이 이 코드로 들어감
		            },
		            error: function (err) {
		                console.log(err);
		            }
